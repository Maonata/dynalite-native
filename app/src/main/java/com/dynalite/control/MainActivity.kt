...
class MainActivity : AppCompatActivity() {
    ...
    private var savedIp
        get() = prefs.getString("ip", "172.18.0.101") ?: "172.18.0.101"
        set(v) { prefs.edit().putString("ip", v).apply() }
    private var savedPort
        get() = prefs.getInt("port", 50000)
    ...
    private fun buildUI(): ScrollView {
        ...
        val btnDiag = Button(this).apply {
            ...
            setOnClickListener {
                startActivity(Intent(this@MainActivity, DiagActivity::class.java))
            }
        }
    ...
    private fun refreshZone(zone: Zone, zui: ZoneUI, remote: Boolean = false) {
        ...
        zui.tvSub.text =
            if (on) "${zone.level}% · A${zone.area} Ch${zone.channel}$remoteTag"
    ...
    private fun syncLevels() {
        ...
        // 2.3 Channel Level Request por cada zona
        zones.forEach { z ->
    ...
    private fun setupClient() {
        ...
        // Eventos de nivel desde PDEG (paneles, otras apps, replies)
        client.onLevelEvent = { event ->
            uiScope.launch {
                if (event.channel == 0) {
                    // CHAN_IDX=FF → ALL channels del área
                    zones.filter { it.area == event.area }.forEach { zone ->
                        val wasOn = zone.isOn
                        ...
                        addLog("REM", "${zone.name} $action (ALL)")
                    }
                } else {
