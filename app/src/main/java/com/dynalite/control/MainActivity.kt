...
class MainActivity : AppCompatActivity() {
    ...
    // ------------------------------------------------------------------
    // ZONES — edita área y canal según tu programación Dynalite
    // ------------------------------------------------------------------
    data class Zone(
        val id: Int,
        val name: String,
    ...
    private var savedIp
        ...
        set(v) {
            prefs.edit().putString("ip", v).apply()
        }
    ...
    private fun buildZoneCard(zone: Zone): ZoneUI {
        ...
        btnToggle.setOnClickListener {
            ...
            addLog(
                "APP",
                "${zone.name} → ${if (zone.isOn) "ON ${zone.level}%" else "OFF"}"
            )
        }
        ...
        client.onLevelEvent = { event ->
            uiScope.launch {
                ...
                if (zone != null) {
                    ...
                    addLog(
                        "RX ",
                        "A${event.area} Ch${event.channel} → ${event.levelPct}%"
                    )
                }
            }
        }
